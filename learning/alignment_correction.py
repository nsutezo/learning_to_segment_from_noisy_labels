...
from dataloader import misaligned_DataGenerator
...
def unet_finetune_vis(epoch,logs=None):
    ...
    if logs.get('val_loss') < my_best:
        ...
        images = []
        alpha = 0.4
        padded_visual_y =  np.zeros((visual_sample_x.shape[0],visual_sample_x.shape[1]+20,visual_sample_x.shape[2]+20,visual_sample_x.shape[3] ))
        padded_visual_y[:,10:visual_sample_x.shape[1]+10,10:visual_sample_x.shape[2]+10,0] = visual_sample_x[:,:,:,0]

        padded_predicted_y =  np.zeros((predicted_y.shape[0],predicted_y.shape[1]+20,predicted_y.shape[2]+20,predicted_y.shape[3] ))
        padded_predicted_y[:,10:predicted_y.shape[1]+10,10:predicted_y.shape[2]+10,:] = predicted_y

        padded_visual_y_true =  np.zeros((visual_sample_x.shape[0],visual_sample_x.shape[1]+20,visual_sample_x.shape[2]+20,visual_sample_x.shape[3] ))
        padded_visual_y_true[:,10:visual_sample_x.shape[1]+10,10:visual_sample_x.shape[2]+10,0] = visual_sample_y[:,:,:,0]
        ...
        for k in range(visual_sample_x.shape[0]):
            ...
            y_contours = measure.find_contours(padded_visual_y_true[k,:,:,0],0.8)
            for ym in y_contours: 
                y_ctr = np.array(ym).reshape((-1,1,2)).astype(np.int32)
                y_ctr_v2 = y_ctr.copy()
                y_ctr_v2[:,:,0] = y_ctr[:,:,1]
                y_ctr_v2[:,:,1] = y_ctr[:,:,0]
                cv2.fillPoly(overlay, [y_ctr_v2-10],(255,0,0))
            ...
            # import ipdb; ipdb.set_trace()
            # note here that blue is misaligned shape and green is prediced shape
...
def get_args():
    ...
    args.trained_models_dir = os.path.join('../results/realignment_trained_models', args.training_time_stamp)
    args.tensorboard_log_dir = os.path.join('../results/realignment_logs', args.training_time_stamp)
...
if __name__ == '__main__':
    args = get_args()
    datagenerator = misaligned_DataGenerator(args)
